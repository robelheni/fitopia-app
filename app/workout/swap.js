...
export default function WorkoutSwapScreen() {
    ...
    async function confirmSwap() {
        ...
        await AsyncStorage.setItem(
            swapKey,
            JSON.stringify({ original: exerciseId, replacement: selected, dayKey: dayKey })
        );
        router.back();
        router.back();
    }
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                {loading ? (
                    ...
                    <View style={styles.loadingContainer}>
                        <Text style={styles.loadingText}>No alternatives available for this exercise.</Text>
                    </View>
                ) : (
                    <FadeUpItem delay={100}>
                        <View style={styles.swapList}>
                            {swaps.map((exercise, index) => (
                                <TouchableOpacity
                                    key={index}
                                    style={[styles.swapCard, selected?.id === exercise.id && styles.swapCardSelected]}
                                    onPress={() => setSelected(selected?.id === exercise.id ? null : exercise)}
                                >
                                    ...
                                    <View style={[styles.checkCircle, selected?.id === exercise.id && styles.checkCircleSelected]}>
...
function makeStyles(c, dark) {
    ...
    return StyleSheet.create({
        ...
        subtitleBold: { fontWeight: '600', color: colors.black },
        ...
        exerciseIcon: {
            ...
            backgroundColor: colors.greyCard, alignItems: 'center', justifyContent: 'center', flexShrink: 0,
        },
        exerciseInfo: { flex: 1, gap: 4 },
        exerciseName: { fontSize: 15, fontWeight: '600', color: colors.black, letterSpacing: -0.3 },
        exerciseMeta: { fontSize: 12, color: colors.grey, fontWeight: '300' },
        exerciseTags: { flexDirection: 'row', gap: 6, flexWrap: 'wrap', marginTop: 2 },
        tag: { backgroundColor: colors.greyCard, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 100 },
        tagText: { fontSize: 10, color: colors.grey, fontWeight: '500' },
        checkCircle: {
