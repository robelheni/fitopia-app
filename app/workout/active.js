...
const quotes = [
    ...
    { text: "I was determined to be the best I could be. That determination never left me.", author: "Tirunesh Dibaba" },
...
const workoutData = {
    'push-day': { name: 'Push Day', exercises: [
    ...
    'pull-day': { name: 'Pull Day', exercises: [
    ...
    'leg-day': { name: 'Leg Day', exercises: [
    ...
    'full-body': { name: 'Full Body', exercises: [
        { id: 'e002', name: 'Push Up', sets: 3, reps: 15, rest: 45, isTimed: false },
        { id: 'e008', name: 'Bodyweight Squat', sets: 3, reps: 15, rest: 45, isTimed: false },
    ...
    'upper-body': { name: 'Upper Body', exercises: [
        { id: 'e001', name: 'Dumbbell Chest Press', sets: 4, reps: 12, rest: 60, isTimed: false },
        { id: 'e006', name: 'Shoulder Press', sets: 4, reps: 12, rest: 60, isTimed: false },
        { id: 'e004', name: 'Dumbbell Row', sets: 4, reps: 10, rest: 60, isTimed: false },
        { id: 'e011', name: 'Bicep Curl', sets: 3, reps: 12, rest: 45, isTimed: false },
    ...
    'lower-body': { name: 'Lower Body', exercises: [
        { id: 'e007', name: 'Dumbbell Squat', sets: 4, reps: 12, rest: 60, isTimed: false },
        { id: 'e008', name: 'Bodyweight Squat', sets: 3, reps: 15, rest: 45, isTimed: false },
        { id: 'e009', name: 'Plank', sets: 3, reps: null, rest: 30, isTimed: true, seconds: 30 },
        { id: 'e010', name: 'Bicycle Crunch', sets: 3, reps: null, rest: 30, isTimed: true, seconds: 30 },
    ]},
    'light-full-body': { name: 'Light Full Body', exercises: [
    ...
    'mobility': { name: 'Mobility', exercises: [
    ...
    'core-and-balance': { name: 'Core and Balance', exercises: [
        { id: 'e009', name: 'Plank', sets: 3, reps: null, rest: 30, isTimed: true, seconds: 30 },
        { id: 'e010', name: 'Bicycle Crunch', sets: 3, reps: null, rest: 30, isTimed: true, seconds: 30 },
    ]},
    'low-intensity-cardio': { name: 'Low Intensity Cardio', exercises: [
        { id: 'e008', name: 'Bodyweight Squat', sets: 2, reps: 12, rest: 60, isTimed: false },
        { id: 'e002', name: 'Push Up', sets: 2, reps: 10, rest: 60, isTimed: false },
        { id: 'e009', name: 'Plank', sets: 2, reps: null, rest: 45, isTimed: true, seconds: 20 },
        { id: 'e010', name: 'Bicycle Crunch', sets: 2, reps: null, rest: 45, isTimed: true, seconds: 20 },
...
export default function ActiveWorkoutScreen() {
    ...
    if (sessionData) {
        ...
        workout = {
            ...
            cardio_circuit: session.cardio_circuit || null,
        };
    } else {
    ...
    useEffect(() => {
        if (workout) {
            progress.value = withTiming((currentIndex / totalExercises) * 100, { duration: 500 });
        }
    }, [currentIndex]);
    ...
    async function handleNextExercise() {
        if (currentIndex + 1 >= totalExercises) {
            if (workout.cardio_circuit && workout.cardio_circuit.exercises?.length > 0) {
                // Navigate to dedicated cardio screen
                router.push({
                    pathname: '/workout/cardio',
                    params: {
                        circuitData: JSON.stringify(workout.cardio_circuit),
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.exerciseHeader}>
                    <Text style={styles.exerciseCount}>Exercise {currentIndex + 1} of {totalExercises}</Text>
                ...
                {resting && (
                    <View style={styles.restCard}>
                        ...
                        <TouchableOpacity style={styles.skipRest} onPress={() => { clearInterval(restInterval.current); setResting(false); }}>
                ...
                {/* Cardio circuit overview */}
                {workout.cardio_circuit && workout.cardio_circuit.exercises?.length > 0 && (
                    <View style={{ marginTop: 12 }}>
                        ...
                        {workout.cardio_circuit.exercises.map((ex, i) => (
                            <TouchableOpacity
                                key={i}
                                style={[styles.overviewItem, { marginBottom: 6 }]}
                                onPress={() => setSelectedExercise({
                                    ...
                                    rest: workout.cardio_circuit.rest_seconds,
                                    instructions: ex.instructions || [],
                                })}
                            >
                                <View style={[styles.overviewDot, { backgroundColor: '#FEE2E2' }]} />
                                <Text style={styles.overviewText}>{ex.name}</Text>
                                <Text style={styles.overviewMeta}>{workout.cardio_circuit.work_seconds}s work</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                )}
            ...
            {/* Next / Start cardio / Finish button */}
...
function makeStyles(c, dark) {
  ...
  return StyleSheet.create({
    ...
    headerProgress: { fontSize: 15, fontWeight: '500', color: colors.grey },
    progressTrack: { height: 4, backgroundColor: colors.greyBorder, marginHorizontal: 24, borderRadius: 2, marginBottom: 8 },
    progressFill: { height: 4, backgroundColor: colors.blue, borderRadius: 2 },
    content: { paddingHorizontal: 24, paddingBottom: 120, paddingTop: 8 },
    exerciseHeader: { marginBottom: 24, paddingTop: 16 },
    exerciseCount: { fontSize: 13, color: colors.grey, fontWeight: '300', marginBottom: 8 },
    exerciseName: { fontSize: 28, fontWeight: '700', color: colors.black, letterSpacing: -0.5, marginBottom: 8 },
    exerciseMeta: { fontSize: 15, color: colors.grey, fontWeight: '300' },
    exerciseNameRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 8 },
    restCard: { backgroundColor: colors.blueLight, borderRadius: 16, padding: 20, alignItems: 'center', marginBottom: 24, borderWidth: 1, borderColor: 'rgba(37,99,235,0.15)', gap: 8 },
    restTitle: { fontSize: 14, fontWeight: '500', color: colors.blueText },
    restTime: { fontSize: 48, fontWeight: '700', color: colors.blueText, letterSpacing: -2 },
    skipRest: { paddingVertical: 6, paddingHorizontal: 16 },
    skipRestText: { fontSize: 13, color: colors.grey },
    setsTitle: { fontSize: 16, fontWeight: '600', color: colors.black, marginBottom: 12 },
    setsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 32 },
    setButton: { width: '47%', paddingVertical: 20, borderRadius: 16, borderWidth: 1.5, borderColor: colors.greyBorder, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center', shadowColor: colors.black, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 8, elevation: 2 },
    setButtonComplete: { backgroundColor: colors.blue, borderColor: colors.blue, shadowColor: colors.blue, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
    setButtonText: { fontSize: 15, fontWeight: '600', color: colors.black },
    overviewTitle: { fontSize: 16, fontWeight: '600', color: colors.black, marginBottom: 12 },
    overviewList: { gap: 8 },
    overviewItem: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: 12, backgroundColor: colors.greyCard },
    overviewItemActive: { backgroundColor: colors.blueLight, borderWidth: 1, borderColor: 'rgba(37,99,235,0.2)' },
    overviewItemDone: { opacity: 0.5 },
    overviewDot: { width: 20, height: 20, borderRadius: 10, backgroundColor: colors.greyBorder, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    overviewDotActive: { backgroundColor: colors.blue },
    overviewDotDone: { backgroundColor: colors.blue },
    overviewText: { fontSize: 14, color: colors.grey, flex: 1, fontWeight: '300' },
    overviewTextActive: { color: colors.blueText, fontWeight: '600' },
    overviewTextDone: { color: colors.grey, textDecorationLine: 'line-through' },
    ...
    modalInstructions: { gap: 12 },
    modalInstructionItem: { flexDirection: 'row', gap: 12, alignItems: 'flex-start' },
    modalInstructionNumber: { width: 26, height: 26, borderRadius: 13, backgroundColor: colors.blueLight, alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 },
    modalInstructionNumberText: { fontSize: 11, fontWeight: '700', color: colors.blueText },
    modalInstructionText: { fontSize: 14, color: colors.black, lineHeight: 22, flex: 1, fontWeight: '300' },
    ...
    completeDoneText: { fontSize: 16, fontWeight: '600', color: colors.blueText },
