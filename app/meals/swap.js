...
export default function SwapScreen() {
  ...
  async function confirmSwap() {
    ...
    await AsyncStorage.setItem(
      swapKey,
      JSON.stringify({ meal: selected, day: day, mealType: mealType, originalMealId: mealId })
    );
    router.back();
  }
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <View style={styles.loadingContainer}>
            <Text style={styles.loadingText}>No alternatives available for this meal.</Text>
          </View>
        ) : (
          <FadeUpItem delay={100}>
            <View style={styles.swapList}>
              {swaps.map((meal, index) => (
                <TouchableOpacity
                  key={index}
                  style={[styles.swapCard, selected?.id === meal.id && styles.swapCardSelected]}
                  onPress={() => setSelected(selected?.id === meal.id ? null : meal)}
                >
                  ...
                  <View style={styles.mealInfo}>
                    ...
                    {meal.swap_reason && <Text style={styles.swapReason}>{meal.swap_reason}</Text>}
                    {meal.tag && (
                  ...
                  <View style={[styles.checkCircle, selected?.id === meal.id && styles.checkCircleSelected]}>
                    {selected?.id === meal.id && (
        ...
        {selected && (
          <FadeUpItem delay={0}>
            <TouchableOpacity style={styles.confirmButton} onPress={confirmSwap}>
              <Feather name="check" size={16} color={'#FFFFFF'} />
              <Text style={styles.confirmText}>Swap to {selected.name}</Text>
            </TouchableOpacity>
          </FadeUpItem>
        )}
      </ScrollView>
...
function makeStyles(c, dark) {
  ...
  return StyleSheet.create({
    ...
    subtitle: { fontSize: 14, color: colors.grey, fontWeight: '300', marginBottom: 20, lineHeight: 20 },
    loadingContainer: { paddingTop: 60, alignItems: 'center', gap: 12 },
    loadingText: { fontSize: 14, color: colors.grey, fontWeight: '300' },
    swapList: { gap: 12, marginBottom: 20 },
    swapCard: {
      flexDirection: 'row', alignItems: 'flex-start', backgroundColor: colors.white, borderRadius: 16, padding: 12,
      borderWidth: 1.5, borderColor: colors.greyBorder,
      shadowColor: colors.black, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 8, elevation: 2, gap: 12,
    },
    swapCardSelected: { borderColor: colors.blue, backgroundColor: colors.blueLight },
    mealImage: { width: 72, height: 72, borderRadius: 12, backgroundColor: colors.greyCard, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    mealInfo: { flex: 1, gap: 4 },
    mealTopRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
    ...
    swapReason: { fontSize: 12, color: colors.grey, fontWeight: '300', fontStyle: 'italic' },
    ...
    checkCircle: { width: 28, height: 28, borderRadius: 14, borderWidth: 1.5, borderColor: colors.greyBorder, alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 4 },
    checkCircleSelected: { backgroundColor: colors.blue, borderColor: colors.blue },
    confirmButton: {
      flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
      backgroundColor: colors.blue, paddingVertical: 16, borderRadius: 100,
